...
export async function addMeeting(
  ...
  const rows = await sql`
    INSERT INTO meetings (
      ...
      ${data.announcements ?? []},
      ${JSON.stringify(data.openingHymn)}, ${data.openingPrayer},
      ${JSON.stringify(data.wardBusiness)}, ${data.stakeBusiness},
      ${JSON.stringify(data.sacramentHymn)},
      ${JSON.stringify(data.speakers)},
      ${JSON.stringify(data.closingHymn)}, ${data.closingPrayer}
    )
    RETURNING
      id,
      to_char(date, 'YYYY-MM-DD') AS "date",
...
export async function updateMeeting(
  ...
  const rows = await sql`
    UPDATE meetings SET
      ...
      opening_hymn = ${JSON.stringify(merged.openingHymn)},
      opening_prayer = ${merged.openingPrayer},
      ward_business = ${JSON.stringify(merged.wardBusiness)},
      stake_business = ${merged.stakeBusiness},
      sacrament_hymn = ${JSON.stringify(merged.sacramentHymn)},
      speakers = ${JSON.stringify(merged.speakers)},
      closing_hymn = ${JSON.stringify(merged.closingHymn)},
